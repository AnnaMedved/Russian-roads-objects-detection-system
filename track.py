...
classes_names = ['fissure', 'large_pit', 'patch', 'small_pit', 'bad_sign', 'graffiti', 'bad_light', 
        'bad_road_mark', 'dirty_stop', 'bad_garden', 'overflowing_bin', 'broken_border']

translates = [
    'Трещина', 'Выбоина', 'Заплатка', 'Малая яма', 'Повреждение опоры', 'Граффити', 'Загрязнение опоры', 
    'Стертая разметка', 'Грязная остановка', 'Плохой сад', 'Излишки мусора', 'Сломанный бордюр'
    ]
...
def run(
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs[i]) > 0:
                        ...
                        name = str(uuid.uuid4())
                        im_save_path = os.path.join(images_fold, name + '.jpg')# im.txt
    ...
    if len(data_pd) > 0: 
        ...
        data_pd = data_pd.drop(
            columns=['Length_mm', 'Width_mm', 'Class_num']
